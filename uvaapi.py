...
def api_get(*args):
    ...
    resp = requests.get("http://uhunt.felix-halim.net/api/" + fmt).text.strip()
...
def get_pid(problem_num):
    return api_get("p/num", problem_num)['pid']

def get_problem_name(problem_num):
    return api_get("p/num", problem_num)['title']
...
def submissions(uid, n):
    return reversed(api_get("subs-user-last", uid, n)["subs"])
...
def user_submissions_problem(user, problem_num):
    uid = get_uid(user)
    resp = api_get("subs-pids", uid, get_pid(problem_num), 0)
    return reverse(resp[uid]["subs"])

def leaderboard(problem_num, n):
    leaders = api_get("p/rank", get_pid(problem_num), 1, n)
    return leaders
